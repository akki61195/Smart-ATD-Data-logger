...
st.markdown("""
    ...
    .stButton>button { background-color: #00d4ff !important; color: black !important; font-weight: bold !important; width: 100% !important; height: 3.8em !important; border-radius: 12px !important; border: none !important; }
    .status-box { padding: 15px; background-color: #1c2128; border: 1px solid #00d4ff; border-radius: 8px; color: #00d4ff !important; font-weight: bold; margin-bottom: 10px; text-align: center; font-size: 1.1rem; }
...
# --- 3. AUTO-LOCATION API (IP Based) ---
def get_location_and_temp():
    try:
        # IP se location nikalna (No Browser Popup)
...
if 'area_name' not in st.session_state: st.session_state.area_name = "Not Detected"
...
# --- 7. AUTO-FETCH BUTTON ---
if st.button("🛰️ AUTO-DETECT LOCATION & TEMP"):
    with st.spinner('Accessing Tower Location...'):
        t, c = get_location_and_temp()
        if t:
            st.session_state.temp_val = t
            st.session_state.area_name = c
            st.rerun()
        else:
            st.error("Network issue! Location access nahi ho pa rahi.")
